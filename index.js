...
if(db.has(`antispam_${message.guild.id}`)){
  antiSpam.check(message, 'kick', 5, 10000);
}
	...
	try {
		if (
			...
			return message.channel.send(
				`<:announce_dark:874348677009920120> **My Prefix In This Server is: \`${prefix}\`**`
			);
		}
	} catch {
		return;
...
bot.on('guildMemberAdd', async member => {
	let MentionUser = member.user;
	let greetchannel = await db.get(`greetchannel__${member.guild.id}`);
	let greetime = await db.get(`greetime_${member.guild.id}`);
	let greetmsg = await db.get(`greetmsg_${member.guild.id}`);
	const testmsg = new MessageEmbed()
...
app.listen(2464, () => console.log(`PROPER FUNCTIONING`));
...
client.login('ODc4MjYyNTA3MDIwNjQ0Mzky.YR-nsw.pYqhlP6X0dymjs2SwT99YLIq-9o');
